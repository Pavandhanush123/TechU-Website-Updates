...
export const Route = createFileRoute("/admin/isa-leads")({
  component: AdminIsaLeads,
});
...
function AdminIsaLeads() {
  ...
  const refresh = useCallback(async () => {
    ...
    else toast.error(leadsRes.error || "Could not load enquiries");
    if (statsRes.ok) {
  ...
  }, [leads, statusFilter, query, dateRange]);
  ...
  const exportLeadsCsv = useCallback(
    (scope: "filtered" | "all") => {
      ...
      if (scope === "filtered") {
        const filterSlug =
          [
            ...
            .filter(Boolean)
            .join("-") || "all-filters";
        a.download = `isa-enquiry-filtered-${filterSlug}-${stamp}.csv`;
      } else {
        a.download = `isa-enquiry-all-${stamp}.csv`;
      }
      a.click();
      ...
      toast.success(
        scope === "filtered"
          ? `Exported ${data.length} enquir${data.length === 1 ? "y" : "ies"} (filtered view)`
          : `Exported ${data.length} enquir${data.length === 1 ? "y" : "ies"} (all loaded)`,
      );
    },
    [filtered, leads, statusFilter, query, dateRange],
  );
  ...
  const handleBulkStatus = async (status: LeadStatus) => {
    ...
    try {
      ...
      toast.success(
        `Updated ${res.updated} enquir${res.updated === 1 ? "y" : "ies"}`,
      );
      setSelected(new Set());
      await refresh();
    } finally {
  ...
  const handleBulkDelete = async () => {
    ...
    if (ids.length === 0) return;
    if (
      !confirm(`Delete ${ids.length} enquir${ids.length === 1 ? "y" : "ies"} permanently?`)
    )
      return;
    setBulkBusy(true);
    try {
      ...
      toast.success(`Deleted ${res.deleted} enquir${res.deleted === 1 ? "y" : "ies"}`);
      setSelected(new Set());
      await refresh();
    } finally {
      setBulkBusy(false);
  ...
  return (
    <AdminShell
      title="ISA Enquiry"
      subtitle="ISA Program enquiries from the homepage popup — course, contact details, and preferred mode."
    >
      {loading ? (
            ...
            <div className="rounded-2xl border border-border bg-card p-3">
              <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                <input
                  ...
                  aria-label="Search enquiries by any field"
                ...
                <div className="flex items-center gap-2 sm:ml-auto">
                  ...
                  <div
                    ...
                    title="Main action uses the filters above. Open the menu to export every loaded enquiry."
                  >
                    <button
                      type="button"
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="min-w-[14rem]">
                        ...
                        >
                          <span>All loaded enquiries</span>
                          <span className="text-xs text-muted-foreground">
                ...
                <tbody className="divide-y divide-border">
                  {filtered.map((l) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className="text-foreground">{l.course}</span>
                      </td>
              ...
              {filtered.map((l) => (
                ...
                >
                  ...
                  <div className="mt-3 space-y-1 text-sm">
                    ...
                    <div className="pt-1 font-medium text-foreground">{l.course}</div>
                  </div>
